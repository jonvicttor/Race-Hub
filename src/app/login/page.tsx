  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        if (error) throw error;
        router.push('/'); // Manda para a home após o login
      }
    } catch (error) {
      // Tipagem segura para erro (silenciando o ESLint)
    ...
    <main className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-6 font-sans">
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center mb-10">
          <div className="bg-race-volt p-4 rounded-2xl mb-4 text-black rotate-3">
        ...
        <form onSubmit={handleAuth} className="flex flex-col gap-4">
          {isSignUp && (
            <div className="relative">
              ...
              <input 
                type="text" placeholder="Seu Nickname (Jones)" required
